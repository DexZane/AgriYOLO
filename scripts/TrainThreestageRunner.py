...
def main():
    ...
    root_runs = "runs/TrainThreestage"

    crop_data = "datasets/Crop/data.yaml"
    pdt_ll_data = "datasets/PDT/LL/PDT.yaml"
    ...
    print("Stage 1: Crop 检测预训练, 适配 12GB 显存")
    print("Stage 2: PDT/LL 单类检测微调")
    print("Stage 3: PDT/LH 单类检测继续微调")

    stage1_weights = os.path.join(root_runs, crop_name, "weights", "best.pt")
    stage2_weights = os.path.join(root_runs, pdt_ll_name, "weights", "best.pt")
    stage3_weights = os.path.join(root_runs, pdt_lh_name, "weights", "best.pt")
    ...
    # ==================== STAGE 1: Crop ====================
    stage1WeightsToUse = resolveBestWeightPath(os.path.join(root_runs, crop_name), stage1_weights)
    ...
    if os.path.exists(stage1_weights):
        ...
        model.train(
            ...
            cache=True,
            patience=crop_patience,
        )

    if not os.path.exists(stage1_weights):
        raise FileNotFoundError(f"❌ Stage 1 未生成权重文件: {stage1_weights}")

    stage1Dir = os.path.join(root_runs, crop_name)
    saveBestInfo(stage1Dir, 'Stage 1 Crop', stage1_weights)
    ...
    # ==================== STAGE 2: PDT/LL ====================
    stage2WeightsToUse = resolveBestWeightPath(os.path.join(root_runs, pdt_ll_name), stage2_weights)
    ...
    print("Stage 1 参数:", {"epochs": crop_epochs, "imgsz": crop_imgsz, "batch": crop_batch, "patience": crop_patience})
